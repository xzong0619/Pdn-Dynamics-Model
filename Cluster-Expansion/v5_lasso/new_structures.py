...
POV_dir_name = 'POV'
CONTCAR_dir_name = 'CONTCAR'

structure_dir = os.path.join(base_dir, output_dir_name, structure_dir_name)
POV_dir = os.path.join(base_dir, output_dir_name, POV_dir_name)
CONTCAR_dir = os.path.join(base_dir, output_dir_name, CONTCAR_dir_name)

run_names = os. listdir(structure_dir)

#%% 


# Read the structures generated by GA
nPd_v =   np.arange(5,22)
...
# Combine different runs the results of different runs
for run in run_names:
    run_dir = os.path.join(structure_dir, run)
    new_Pd_run[run] =  dict()
    for Pdi in nPd_v:
        try: 
            [new_Pd_run[run][Pdi], _] = pickle.load(open(os.path.join(run_dir, 'pd_'+str(Pdi)+".p"), "rb"))
        except:
            tools, base, creator = GA.import_deap() 

# Save to one dictionary
for key in list(nPd_v):
    new_Pd[key] = [] 
    for run in run_names:
        new_Pd[key] += new_Pd_run[run][key]
        
# Take out the repeated elements
for key in list(nPd_v):
    _, unique_indices = np.unique(new_Pd[key], return_index = True, axis = 0)
    unique_s = [new_Pd[key][i] for i in unique_indices]
    new_Pd[key], new_E[key]  = GA.hall_of_fame(COMM = None, history = unique_s, nbest = len(unique_s), output = False)

# Rerank all the individuals and take the energy value
...
for Pdi in nPd_v:
    for index in range(new_count_dict[Pdi]):
        save_CONTCAR(Pdi, index+1, new_atoms_dict[Pdi][index], CONTCAR_dir)
        save_POV(Pdi, index+1, new_atoms_dict[Pdi][index], POV_dir)